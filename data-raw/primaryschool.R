# Load data
con <- gzcon(url("http://www.sociopatterns.org/wp-content/uploads/2015/09/primaryschool.csv.gz"))
txt <- readLines(con)
records <- read.delim(textConnection(txt), header = FALSE)

con <- gzcon(url("http://www.sociopatterns.org/wp-content/uploads/2015/09/metadata_primaryschool.txt"))
txt <- readLines(con)
...
# Save
usethis::use_data(primaryschool, overwrite = TRUE)
