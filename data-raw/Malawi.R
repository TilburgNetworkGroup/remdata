# Load data
con <- gzcon(url("http://www.sociopatterns.org/wp-content/uploads/2021/09/tnet_malawi_pilot.csv.gz"))
txt <- readLines(con)
records <- read.csv(textConnection(txt), row.names = NULL)
records <- records[,-1]
colnames(records) <- c("time", "day", "actor1", "actor2")
...
# Create a risk set
actors <- with(records, sort(unique(c(actor1, actor2))))
...
# Collect data
Malawi <- records[records$onset==TRUE,
  c("time", "actor1", "actor2", "duration", "day")]
rownames(Malawi) <- NULL
