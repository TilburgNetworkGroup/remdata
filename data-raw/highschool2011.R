# Load data
con <- gzcon(url("http://www.sociopatterns.org/wp-content/uploads/2014/08/highschool_2011.csv.gz"))
txt <- readLines(con)
records <- read.delim(textConnection(txt), header = FALSE)

con <- gzcon(url("http://www.sociopatterns.org/wp-content/uploads/2015/09/metadata_2011.txt"))
txt <- readLines(con)
...
highschool2011 <- list(edgelist = edgelist, attributes = attributes)

usethis::use_data(highschool2011, overwrite = TRUE)
